  ...
  <div>
    <a-spin :spinning="spinning" :tip="tip">
          ...
          <a-row>
            ...
            <a-col :span="6">
              <div style="margin-left:10px;">
                <img
                  v-if="item.selectedPicUrl"
                  :src="item.selectedPicUrl"
                  alt="物料图片"
                  title="查看大图"
                  @click="() => handleImgView(item.selectedPicUrl)"
      ...
      <p style="margin-top:20px;text-align:center;">
        <a-button type="primary" @click="onSubmit" :disabled="disabledStepButton">
          下一步
        </a-button>
  ...
  methods: {
    ...
    async qrChangeHandler(dataUrl, id) {
      ...
      if (dataUrl) {
        ...
        //   },150)
        // })
        let file = that.Base64toFile(dataUrl, 'wuliao_qr.png')
        if (file !== null) {
          const formData = new FormData()
          formData.append('file', file)
          let url = await customUpload(formData)
            .then(res => res.data)
            .catch(err => {
    ...
    makeMaterialCode(joinSymbol = '.') {
      ...
      let spec = arr.length > 0 ? arr.join(joinSymbol) : ''
    ...
    initSpecifications(node) {
      const that = this
      that.spinning = true
      return routineMaterialRulePageTwoTierTreeList({ parentId: node.value, type: 3 })
        .then(res => {
          that.spinning = false
          // console.log(res)
          if (!res || res.code !== 200) {
