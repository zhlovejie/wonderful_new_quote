    ...
    <a-spin :spinning="spinning">
      <div class="search-wrapper">
        <a-form layout="inline">
          <a-form-item>
            <a-input
              placeholder="检验方案模糊查询"
              v-model="queryParam.inspectionScheme"
              allowClear
              style="width: 200px"
            />
          </a-form-item>
...
import {
  ...
  checkInspectionSchemeSetDelete,
  checkInspectionSchemeSetDetail,
  checkInspectionSchemeSetPage,
...
export default {
  ...
  watch: {
    $route: {
      handler: function(to, from) {
        if (to.name === 'quality-management_standard_inspection-scheme-set') {
          this.init()
        }
      },
      immediate: true
  ...
  methods: {
    ...
    search(params = {}) {
      ...
      that.loading = true
      checkInspectionSchemeSetPage(_searchParam)
        .then(res => {
          that.loading = false
    ...
    doAction(type,record){
      ...
      if(['add','view','edit'].includes(type)){
        ...
        checkInspectionSchemeSetDelete({id:record.id}).then(res => {
          if(res.code === 200){
            that.search()
          }else{
