...
import { quality_trust_pageList, quality_getCheckGetMaterialId, finishCheck } from '@/api/qualityTesting'
...
export default {
  ...
  data() {
    return {
      ...
      loadData: (parameter) => {
        return quality_trust_pageList(Object.assign(parameter, this.queryParam)).then((res) => {
          return res
        })
      },
    }
  ...
  watch: {
    $route(to, from) {
      if (to.name == 'quality-management_Warehousing_Outsourcing') {
        this.search()
      }
    },
  },
