  ...
  >
    <a-form :form="form" class="search-form-wrapper">
      <a-row :gutter="0">
        ...
        <a-col :span="12">
          <a-form-item label="报废申请单号">
            <a-input v-decorator="['scrapNum']" placeholder="收料单号" :allowClear="true" />
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label="报检部门">
            <a-select placeholder="选择部门" v-decorator="['scrapDepartmentId']" :allowClear="true">
...
export default {
  ...
  methods: {
    query(tabKey) {
      ...
      this.visible = true
      departmentList().then((res) => (this.saleUser = res.data))
    },
    handleCancel() {
      this.visible = false
    },
