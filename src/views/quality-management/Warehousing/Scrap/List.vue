...
import { quality_scrap_pageList, quality_getCheckGetMaterialId } from '@/api/qualityTesting'
...
export default {
  ...
  data() {
    return {
      ...
      loadData: (parameter) => {
        return quality_scrap_pageList(Object.assign(parameter, this.queryParam)).then((res) => {
          return res
        })
      },
    }
  ...
  watch: {
    $route(to, from) {
      if (to.name == 'quality-management_Warehousing_Scrap') {
        this.search()
      }
    },
  },
