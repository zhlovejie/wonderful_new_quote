      ...
      <a-col>
        ...
        >
          ...
          <div slot="checkType" slot-scope="text, record">
            <span v-if="text == 1">半成品检验</span>
            <span v-if="text == 2">成品检验</span>
          </div>
...
import { quality_finish_pageList, quality_getCheckGetMaterialId } from '@/api/qualityTesting'
...
export default {
  ...
  data() {
    return {
      ...
      loadData: (parameter) => {
        return quality_finish_pageList(Object.assign(parameter, this.queryParam)).then((res) => {
          return res
        })
      },
    }
  ...
  watch: {
    $route(to, from) {
      if (to.name == 'quality-management_Warehousing_Processt') {
        this.search()
      }
    },
  },
