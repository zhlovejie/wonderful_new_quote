        ...
        <table class="custom-table custom-table-border">
          <tbody>
            <tr>
              ...
              <td>
                <a-form-model-item>
                  {{ { 1: '半成品检验', 2: '成品检验' }[CheckDetail.checkType] }}
                </a-form-model-item>
              </td>
              <td style="width: 150px">
                <span>紧急程度</span>
...
import { quality_finish_getCheckDetail, quality_finish_dealCheck } from '@/api/qualityTesting'
...
export default {
  ...
  methods: {
    ...
    query(type, record = {}, test) {
      ...
      quality_finish_getCheckDetail({ checkId: record.id }).then((res) => {
    ...
    handleSubmit(operationType) {
      ...
      that.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          ...
          console.log(params)
          quality_finish_dealCheck(params)
            .then((res) => {
              that.spinning = false
