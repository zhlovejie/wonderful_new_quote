    ...
    <div class="top-ation">
      <a-form layout="inline">
        <a-form-item>
          <a-input
            style="width: 200px"
            placeholder="邮寄单编号模糊查询"
            allowClear
            v-model="queryParam.mailNum"
          />
        </a-form-item>
        <a-form-item>
          <a-input
...
export default {
  ...
  watch: {
    $route: {
      handler: function(to, from) {
        if (to.name === 'after-sales-management_return-goods_unreceives-customer') {
          this.search()
        }
      },
      immediate: true
  ...
  methods: {
    search(params = {}) {
      ...
      receiveCustomerNotReceiveList(_searchParam)
        .then(res => {
          that.loading = false
