<template>
  ...
  <div class="adjust-apply-list-wrapper">
    <div class="search-wrapper">
      <a-input
        placeholder="合同编号"
        v-model="queryParam.accessoriesContractNum"
        allowClear
    ...
    <div class="main-wrapper">
      <a-tabs :activeKey="String(activeKey)" defaultActiveKey="0" @change="tabChange">
        ...
        <template v-if="$auth('Accessories:list')">
          <a-tab-pane tab="待我审批的" key="1" />
          <a-tab-pane tab="我已审批的" key="2" />
        </template>
      </a-tabs>
      <a-table
        v-if="$auth('Accessories:lists')"
        :columns="columns"
      ...
      >
        ...
        <div class="action-btns" slot="action" slot-scope="text, record">
          ...
          <template v-if="activeKey === 0">
            <template v-if="$auth('Accessories:view')">
              <a type="primary" @click="doAction('veiw', record)">查看</a>
            </template>
            <template v-if="record.status === 1 && +record.createdId === +userInfo.id">
              <a-divider type="vertical" />
              <template v-if="$auth('Accessories:Withdraw')">
...
<script>
import {
  accessoriesgetListByPage,
  accessoriesRevocation,
  accessoriesdelById,
  translocateIsEnd,
...
const innerColumns = [
  ...
  { align: 'center', title: '物料名称', dataIndex: 'materialName', width: 150 },
  { align: 'center', title: '规格型号', dataIndex: 'specification', width: 320 },
  { align: 'center', title: '单位', dataIndex: 'company' },
  { align: 'center', title: '数量', dataIndex: 'quantity' },
  { align: 'center', title: '单价（元）', dataIndex: 'unitPrice', width: 150 },
  { align: 'center', title: '金额（元）', dataIndex: 'money', scopedSlots: { customRender: 'money' } },
]
...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'after-sales-management_Accessories') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    confirmDelete(record) {
      let that = this
      accessoriesdelById({ id: record.id }).then((res) => {
        if (res.code === 200) {
          this.searchAction()
    ...
    confirmWithdraw(record) {
      let that = this
      accessoriesRevocation(`id=${record.id}`).then((res) => {
        this.searchAction()
        that.$message.info(res.msg)
      })
    },
    ...
    searchAction(opt) {
      ...
      that.loading = true
      accessoriesgetListByPage(_searchParam)
        .then((res) => {
          that.loading = false
