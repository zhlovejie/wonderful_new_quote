  ...
  <a-spin :spinning="spinning">
    <div class="monitor-index-wrapper">
      <div class="bar-chart" :style="{ minHeight: height + 'px' }">
        <a-row :gutter="20">
          <a-col :span="8" :style="{ borderRight: '1px solid #eee' }">
            ...
            <template v-else>
              <p class="pie-desc" :style="{ height: height + 'px', lineHeight: height + 'px', color: '#999' }">
                暂无数据
              </p>
            </template>
          </a-col>
          <a-col :span="8" :style="{ borderRight: '1px solid #eee' }">
            ...
            <template v-if="memoryData.length > 0">
              ...
              <p class="pie-desc">
                ...
                <span>运行内存：{{ memUs }}</span>
                <span>剩余内存：{{ memSp }}</span>
                <span>缓存：{{ memCa }}</span>
              </p>
            </template>
            <template v-else>
      ...
      <div class="line-chart" :style="{ minHeight: height + 'px' }">
        <a-row>
          <a-col>
            <h3 style="text-align: center; padding: 20px 0; margin-bottom: 20px; background-color: #f9f9f9">
              在线人数监控
            </h3>
            <div class="search-wrapper">
              <span class="online-tips"
                >当前在线&nbsp;&nbsp;移动端&nbsp;{{ onlinePerson.onlineNumberPhone }}&nbsp;人&nbsp;&nbsp;PC端&nbsp;{{
            ...
            <template v-if="lineData.length > 0">
              <v-chart :forceFit="true" :height="height" :data="lineData" :scale="lineScale" :padding="padding">
                ...
                <v-guide type="dataMarker" :position="markerPosition" :content="markerContent" :style="style">
                </v-guide>

                <v-guide type="dataMarker" :position="markerMinPosition" :content="markerMinContent" :style="style">
                </v-guide>
            ...
            <template v-else>
              <p class="pie-desc" :style="{ height: height + 'px', lineHeight: height + 'px', color: '#999' }">
                暂无 {{ onlineDate.format('YYYY-MM-DD') }} 的数据
              </p>
            </template>
          </a-col>
        </a-row>
...
export default {
  ...
  methods: {
    async init() {
      ...
      if (CPU) {
        ...
        this.cpuData = new DataSet.View()
          ...
          .transform({ type: 'percent', field: 'count', dimension: 'item', as: 'percent' }).rows
      } else {
        this.cpuData = []
      }
      if (MEM) {
        ...
        this.memSum = this.formatSize((parseInt(MEM['memSum']) || 0) * 1024 * 1024)
        this.memUs = this.formatSize((parseInt(MEM['memUs']) || 0) * 1024 * 1024)
        this.memSp = this.formatSize((parseInt(MEM['memSp']) || 0) * 1024 * 1024)
        this.memCa = this.formatSize((parseInt(MEM['memCa']) || 0) * 1024 * 1024)
    ...
    fetchOnlineData() {
      ...
      return getOnlineUser({ date: _date })
        .then((res) => res.data)
        .catch((err) => {
          console.log(err)
          return []
      ...
      */
      ...
      if (onlineDate.length > 0) {
        ...
        this.lineData = new DataSet.View()
          ...
          .transform({ type: 'fold', fields: ['移动端', '电脑端'], key: 'onlineNumber', value: 'persons' }).rows
      } else {
        this.lineData = []
      }
      this.spinning = false
