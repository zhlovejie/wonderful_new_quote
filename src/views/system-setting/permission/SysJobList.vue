  ...
  <a-card :bordered="false">
    <a-form layout="inline">
      <a-form-item label="任务名称">
        <a-input v-model.trim="queryParam.jobName" :allowClear="true" />
      </a-form-item>
      <a-form-item label="任务组">
        <a-input v-model.trim="queryParam.jobGroup" :allowClear="true" />
      </a-form-item>
      <a-form-item label="bean名称">
        <a-input v-model.trim="queryParam.beanName" :allowClear="true" />
      </a-form-item>
      <a-form-item>
      ...
      <a-form-item>
        ...
        <a-button v-if="$auth('sysJob:list')" style="margin-left: 8px" type="primary" @click="doAllJob('pause')"
          >暂停全部</a-button
        >
        <a-button v-if="$auth('sysJob:list')" style="margin-left: 8px" type="primary" @click="doAllJob('resume')"
          >启动全部</a-button
        >
...
export default {
  ...
  data() {
    return {
      ...
      loadData: (parameter) => {
        ...
        return listPage(Object.assign(parameter, this.queryParam), this.research)
          .then((res) => {
            return res
          })
          .catch((error) => {
  ...
  methods: {
    // 新增
    handleAdd() {
      this.$refs.modal.add(this.research)
    },
    // 修改详情
    handleEdit(e) {
      this.$refs.editModal.edit(e, this.research)
    },
    // 详情
    query(e) {
      this.$refs.queryModal.query(e, this.research)
    },
    // 删除
    delJob(record) {
      const _this = this
      this.$confirm({
        ...
        onOk() {
          // 在这里调用删除接口
          del(`quartzId=${record.id}`, _this.research)
            .then((data) => {
              if (data.code == 200) {
    ...
    doOneJob(actionType, record) {
      ...
      if (actionType === 'pause') {
        //meetingRecordCancel(`id=${record.id}`)
        pauseOne(`quartzId=${record.id}`, this.research)
          .then((data) => {
            if (data.code == 200) {
        ...
        resumeOne(`quartzId=${record.id}`, this.research)
          .then((data) => {
            if (data.code == 200) {
