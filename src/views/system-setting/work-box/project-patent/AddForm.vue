    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="专利类别" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select
            placeholder="请选择专利类别"
            v-decorator="['dicId', {initialValue:detail.dicId, rules: [{ required: true, message: '请选择专利类别' }] }]"
          >
        ...
        <a-form-item label="专利名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['patentName', {initialValue:detail.patentName, rules: [{ required: true, message: '请输入专利名称' }] }]"
          />
        </a-form-item>
        <a-form-item label="专利号" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['patentNum', { initialValue:detail.patentNum,rules: [{ required: true, message: '请输入专利号' }] }]"
          />
        </a-form-item>
        <a-form-item label="专利申请日" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-date-picker
            placeholder="专利申请日"
            v-decorator="['applicationDate',{initialValue:detail.applicationDate ? moment(`${detail.applicationDate}`) :undefined,rules: [{required: true,message: '选择专利申请日'}]}]"
            style="width:100%;"
            format="YYYY-MM-DD"
          />
        </a-form-item>
        ...
        <a-form-item label="专利描述" :labelCol="labelCol" :wrapperCol="wrapperCol">
          ...
          <a-input
            type="hidden"
            v-decorator="['patentDescription', {initialValue:detail.patentDescription, rules: [{ required: false, min: 5, message: '请填写专利描述！' }] }]"
          />
        </a-form-item>
      </a-form>
    </a-spin>
...
<script>
import { addAndUpdateProjectPatent, getProjectPatentDetail } from '@/api/workBox'
    ...
    handleSubmit() {
      ...
      validateFields(async (errors, values) => {
        if (!errors) {
          ...
          addAndUpdateProjectPatent(values)
            .then((res) => {
