      ...
      <a-form :form="form" >
        ...
        <a-form-item
          ...
          <a-input
            ...
            v-decorator="['versionName', {
              rules: [{
                type: 'string', message: '请输入正确版本名称!',
              }, {
                required: true, message: '请输入正确版本名称!',
              }]
            }]"/>
        </a-form-item>
        <a-form-item
          ...
          <a-input
            :disabled="queryBoolean"
            placeholder="请输入版本号"
            v-decorator="['versionCode', {
              rules: [{
                type: 'string', message: '请输入正确版本号!',
              }, {
                required: true, message: '请输入正确版本号!',
              }]
            }]"/>
        </a-form-item>
        <a-form-item
        ...
        <a-form-item label="文件" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-upload
            ...
            @change="handleChange">
            <a-button>
              <a-icon type="upload" /> 上传（.apk类型）
            </a-button>
            <a-modal :visible="previewVisible" :footer="null" @cancel="previewCancel">
            </a-modal>
          </a-upload>
          <a-input type="hidden" v-decorator="['downloadUrl', {rules: [{required: true,message: '请选择文件！'}]}]"/>
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
...
import { uploadAndroidApk, deleteAndroidVersion, editAndroidVersion, addAndroidVersion } from '@/api/workBox'
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          if (_this.addOredit == 'save') {
            ...
            editAndroidVersion(values).then((data) => {
              if (data.code == 200) {
