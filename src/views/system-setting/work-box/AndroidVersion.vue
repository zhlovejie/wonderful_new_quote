...
import { uploadAndroidApk, deleteAndroidVersion, editAndroidVersion, addAndroidVersion, listAndroidVersion } from '@/api/workBox'
  ...
  methods: {
    ...
    del (record) {
      ...
      this.$confirm({ title: '警告',
        ...
        onOk () {
          // 在这里调用删除接口
          deleteAndroidVersion({ 'versionId': record.id }).then(data => {
            if (data.code == 200) {
