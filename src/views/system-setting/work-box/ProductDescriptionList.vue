  ...
  <a-card :bordered="false">
    <div class="description-document-search-wrapper">
      <a-form layout="inline">
        ...
        <template v-if="$auth('ProductDescription:list')">
          <a-form-item>
        ...
        <div class="action-wrapper" style="float:right;">
          <a-form-item>
            <template v-if="$auth('ProductDescription:add')">
        ...
        >
          ...
          <span slot="action" slot-scope="text, record">
            ...
            <template v-if="$auth('ProductDescription:edit')">
              <a-divider type="vertical"/>
              <a @click="handleAction('edit',record)">编辑</a>
            </template>
            <template v-if="$auth('ProductDescription:del')">
              <a-divider type="vertical"/>
  ...
  methods: {
    handleAction(type,record){
      ...
      if(['add','edit'].includes(type)){
        ...
        that.$refs.toolBoxCommonUploadForm.query(type,_record,8,0)  
      }else if(type === 'del'){
