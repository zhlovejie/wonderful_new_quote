      ...
      <a-form :form="form" layout="inline"  class="wdf-custom-add-form-wrapper">
        ...
        <table class="custom-table custom-table-border">
          ...
          <tr>
            ...
            <td>
              <a-form-item>
                <a-date-picker v-if="isAdd || isEdit" @change="dateChange"  placeholder="开始日期" v-decorator="['beginDate',{initialValue:moment(detail.beginDate),rules: [{required: true,message: '请选择开始日期'}]}]" style="width:100%;" />
          ...
          <tr>
            ...
            <td>
              <a-form-item>
                <a-date-picker v-if="isAdd || isEdit" @change="dateChange" placeholder="结束日期" v-decorator="['endDate',{initialValue:moment(detail.endDate),rules: [{required: true,message: '请选择结束日期'}]}]" style="width:100%;" />
          ...
          <tr>
            ...
            <td>
              <a-form-item>
                ...
                />
                <span v-else>{{detail.informContent}}</span>
              </a-form-item>
            </td>
          </tr>
        </table>
...
import {
  festivalRuleDetail , 
  festivalRuleAddAndUpdate ,
  festivalRuleApproval
...
export default {
  ...
  components:{Approval,MultipleCalendarDates},
  data(){
    return {
    ...
    handleSubmit(){
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          values.changeRestDate = Array.isArray(that.changeRestDate) && that.changeRestDate.length > 0 
            ? that.changeRestDate.join(',')
            : undefined
          
          console.log('Received values of form: ', values)

          festivalRuleAddAndUpdate(values).then(res =>{
