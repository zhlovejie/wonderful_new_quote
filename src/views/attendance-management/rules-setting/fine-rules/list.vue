...
<script>
import { getFineRuleList, delFineRule } from '@/api/attendanceManagement'
// import { customerPunishRuleDetail, customerPunishRuleDelete } from '@/api/customerReleaseRule'
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'attendance-rules-setting-fine-rules') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    searchAction(opt = {}) {
      ...
      getFineRuleList()
        .then((res) => {
          that.loading = false
          //   let result = (res.data.records = res.data.records ? [res.data.records] : [])
