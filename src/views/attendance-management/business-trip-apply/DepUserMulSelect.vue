  ...
  methods: {
    ...
    userAction(type) {
      ...
      if (type === 'add') {
        ...
        that.joinList.push(Object.assign({}, that.current, { _key: makeUUID() }))
      } else if (type === 'reset') {
        that.joinList = []
      }
    },
    removeTag(item) {
      this.joinList = this.joinList.filter(p => p._key !== item._key)
    }
  }
}
</script>
