        ...
        <table class="custom-table custom-table-border">
            ...
            <td>
              <a-form-item>
                <a-select
                  ...
                  v-decorator="[
                    'holidayId',
                    { initialValue: detail.holidayId, rules: [{ required: true, message: '请选择请假类型' }] },
                  ]"
                  :allowClear="true"
                  style="width: 100%"
          ...
          <tr>
            ...
            <td>
              <a-form-item>
                ...
                />
                <span v-else>{{
                  +detail.holidayUnitType === 1 ? detail.beginTime.slice(0, 10) : detail.beginTime
                }}</span>
              </a-form-item>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <a-form-item
                >{{ leaveTime
                }}{{ holidayTarget ? { 1: '天', 3: '小时' }[holidayTarget.holidayUnitType] || '' : '' }}</a-form-item
              >
            </td>
          </tr>
            ...
            <td>
              <a-form-item>
                <a-textarea
                  ...
                  v-decorator="[
                    'reason',
                    { initialValue: detail.reason, rules: [{ required: true, message: '请输入请假事由' }] },
                  ]"
                />
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      let task1 = attenceLeaveApplyAllHoliday().then((res) => {
        if (that.userInfo.stationType === 4) {
          that.holidayList = res.data.filter((item) => {
            return item.holidayName === '事假' || item.holidayName === '病假'
          })
        } else {
    ...
    disabledDate(current) {
      ...
      if (beginTime instanceof moment && this.holidayTarget !== null && this.holidayTarget.legalDuration > 0) {
    ...
    async query(type, record) {
      ...
      that.spinning = true
      await attenceLeaveApplyDetail({ id: record.id })
        .then((res) => {
          that.spinning = false
          //debugger
          let data = res.data
