            ...
            <td>
              <a-form-item>
                <a-select
                  ...
                  v-decorator="[
                    'exceptionId',
                    { initialValue: detail.exceptionId, rules: [{ required: true, message: '请选择加班事件' }] },
                  ]"
                  :allowClear="true"
                  style="width: 100%"
            ...
            <td>
              <a-form-item>
                <a-textarea
                  ...
                  v-decorator="[
                    'reason',
                    { initialValue: detail.reason, rules: [{ required: true, message: '请输入加班原因' }] },
                  ]"
                />
                <span v-else>
                  {{ detail.reason }}
...
export default {
  ...
  methods: {
    ...
    fetchOverworkApplyHours({ beginTime, endTime }) {
      let that = this
      if (!this.isView && !this.isApproval) {
        overworkApplyHours({
          beginTime,
          endTime,
        }).then((res) => {
    ...
    handleSubmit() {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          overworkApplyAddAndUpdate(values)
            .then((res) => {
              that.$message.info(res.msg)
              that.spinning = false
              that.handleCancel()
