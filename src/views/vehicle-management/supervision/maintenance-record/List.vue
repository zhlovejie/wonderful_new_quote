...
import { careHistoryList } from '@/api/vehicleManagement'
...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'vehicle-management-supervision-maintenance-record') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    doAction(type, record) {
      let that = this
      if (type === 'evidence') {
        if (record.careEvidenceList.length > 0) {
          let imgList = record.careEvidenceList.map((item) => {
