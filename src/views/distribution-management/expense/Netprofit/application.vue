...
<script>
import {
  logisticsgetAliPayQrCode,
  logisticsgetWxPayQrCode,
  // accessoriesManagementDetail,
} from '@/api/distribution-management'
...
export default {
  ...
  methods: {
    ...
    fillData() {
      ...
      logisticsgetAliPayQrCode(id).then((res) => {
        that.recordDetails = res.data
      })
      logisticsgetWxPayQrCode(id).then((res) => {
