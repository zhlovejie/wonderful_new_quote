...
import InvestigateNode from './modules/InvestigateNode'
import Tendering from './modules/TenderingUnit'
...
export default {
  ...
  data() {
    return {
      ...
      loadData: (parameter) => {
        return getInvoiceList(Object.assign(parameter, this.queryParam)).then((res) => {
          return res
        })
      },
      selectedRowKeys: [],
  ...
  watch: {
    $route(to, from) {
      ...
      if (to.name === 'distribution_Invoice') {
        this.dayWeekMonth = 1
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push({ name: 'distribution_InvoiceModules', params: { type: 'Add' } })
    },
    handleSee(e) {
      this.$router.push({ name: 'distribution_InvoiceApproval', params: { record: e, type: 'See' } })
    },
    handleApproval(e) {
      this.$router.push({ name: 'distribution_InvoiceApproval', params: { record: e, type: 'Approval' } })
    },
    handleAuditOk() {
      this.searchAction()
    },
    ...
    updateInvoice(record) {
      this.$router.push({ name: 'distribution_InvoiceModules', params: { record, type: 'Edit' } })
    },
    openModel() {
