  ...
  <a-card :bordered="false">
    ...
    <div class="description-document-search-wrapper">
      <a-form layout="inline">
        ...
        <a-form-item>
          <a-input v-model.trim="queryParam.planeName" allowClear placeholder="平面库位名称" />
        </a-form-item>
        <a-form-item>
          <a-input v-model.trim="queryParam.planeCode" allowClear placeholder="平面库位代码" />
        </a-form-item>
        <template>
          <a-form-item>
...
<script>
import { getList, planegetListByPage, planegetdelValidation, planegetdelById, ReservoiGetList } from '@/api/storage'
  ...
  data() {
    return {
      ...
      loadData: (parameter) => {
        ...
        return planegetListByPage(Object.assign(parameter, this.queryParam))
          .then((res) => {
            return res
          })
          .catch((error) => {
  ...
  methods: {
    ...
    del(record) {
      ...
      that.confirmModel({
        content: '确认删除该条数据吗?',
        success: () => {
          planegetdelValidation({ id: record.id })
            .then((res) => {
              if (+res.code === 200) {
                that.delvali(record)
              } else {
