<template>
  ...
  >
    <template slot="footer">
      ...
      <a-button key="submit" type="primary" :loading="spinning" @click="handleOk">确定收料</a-button>
    </template>
      ...
      <a-form-model ref="ruleForm" :model="form" class="routine-addform-wrapper-baseInnerData">
        ...
        <table class="custom-table custom-table-border">
          <tr>
            ...
            <td style="width: 25%">
              ...
              >
                <a-select v-model="form.warehouseId" placeholder="选择仓库">
        ...
        <div style="margin-top: 20px">
          ...
          <a-table :columns="columns" :dataSource="form.batchListBoList" :pagination="false" bordered size="small">
            ...
            <div slot="recieveCount" slot-scope="text, record, index">
              <a-form-model-item
                :prop="'batchListBoList.' + index + '.recieveCount'"
                :rules="{
                  required: true,
                  message: '请输入数量',
                }"
              >
                <a-input-number :min="0" :step="1" :precision="0" v-model="record.recieveCount" />
              </a-form-model-item>
            </div>
...
export default {
  ...
  methods: {
    ...
    query(type, record) {
      ...
      if (type === 'Batch') {
        this.form.batchListBoList = [...record]
      }
      if (type === 'Receiving') {
