<template>
  ...
  <div class="adjust-apply-list-wrapper">
    <div class="search-wrapper">
      <a-input
        placeholder="调拨单编号"
        v-model="queryParam.allocateNum"
        allowClear
      ...
      <a-button
        ...
        >查询</a-button
      >
      <template v-if="$auth('alloction:add')">
    ...
    <div class="main-wrapper">
      <a-tabs :activeKey="String(activeKey)" defaultActiveKey="0" @change="tabChange">
        ...
        <template v-if="$auth('alloction:list')">
          <a-tab-pane tab="待我审批的" key="1" />
          <a-tab-pane tab="我已审批的" key="2" />
        </template>
      </a-tabs>
      <a-table
        v-if="$auth('alloction:lists')"
        :columns="columns"
        ...
        <div class="action-btns" slot="action" slot-scope="text, record">
          ...
          <template v-if="activeKey === 0">
            ...
            <template v-if="record.status === 2 && +record.createdId === +userInfo.id">
              <a-divider type="vertical" />
              <template v-if="$auth('alloction:WithdrawEnd')">
...
<script>
import { allocategetListByPage, allocaterevocation, allocatedelById, allocateIsEnd } from '@/api/storage'
...
const innerColumns = [
  ...
  },
  {
    align: 'center',
    title: '移入仓库/库区',
    dataIndex: 'immigrateWarehouseReservoir',
    key: 'immigrateWarehouseReservoir',
  },
  {
    align: 'center',
    title: '移入库位',
...
export default {
  ...
  methods: {
    ...
    confirmDelete(record) {
      let that = this
      allocatedelById({ id: record.id }).then((res) => {
        if (res.code === 200) {
          this.searchAction()
    ...
    confirmEnd(record) {
      let that = this
      allocateIsEnd({ id: record.id }).then((res) => {
        this.searchAction()
        that.$message.info(res.msg)
      })
    },
    ...
    confirmWithdraw(record) {
      let that = this
      allocaterevocation({ id: record.id }).then((res) => {
        this.searchAction()
        that.$message.info(res.msg)
      })
    },
    ...
    searchAction(opt) {
      ...
      that.loading = true
      allocategetListByPage(_searchParam)
        .then((res) => {
          that.loading = false
