<template>
  <div class="adjust-apply-list-wrapper">
    ...
    <div class="search-wrapper">
      <template v-if="$auth('Specification:add') && dataSource.length === 0 && this.activeKey === 0">
    ...
    <div class="main-wrapper">
      <a-tabs :activeKey="String(activeKey)" defaultActiveKey="0" @change="tabChange">
        ...
        <template v-if="$auth('Specification:approve')">
          <a-tab-pane tab="待我审批" key="1" />
          <a-tab-pane tab="我已审批" key="2" />
        </template>
      </a-tabs>
      <a-table
        v-if="$auth('Specification:lists')"
        :columns="columns"
      ...
      >
        ...
        <div class="action-btns" slot="action" slot-scope="text, record">
          <template v-if="activeKey === 0">
            <template v-if="$auth('Specification:view')">
              <a type="primary" @click="doAction('view', record)">查看</a>
            </template>
            <template v-if="(record.status === 1 && +record.createdId === +userInfo.id) || record.status === 2">
              <a-divider type="vertical" />
              <template v-if="$auth('Specification:Withdraw')">
                <a-popconfirm title="是否确定撤回" ok-text="确定" cancel-text="取消" @confirm="confirmWithdraw(record)">
                  <a type="primary">撤回</a>
                </a-popconfirm>
              </template>
            </template>
            <template v-if="$auth('Specification:edit-salary') && (record.status === 3 || record.status === 4)">
              <a-divider type="vertical" />
...
<script>
import { productRulegetListByPage, revocation, productdelById } from '@/api/storage'
...
export default {
  ...
  methods: {
    ...
    confirmWithdraw(record) {
      let that = this
      revocation({ id: record.id }).then((res) => {
        this.searchAction()
    ...
    confirmDelete(record) {
      let that = this
      productdelById({ id: record.id }).then((res) => {
        if (res.code === 200) {
          this.searchAction()
          that.$message.info(res.msg)
        } else {
    ...
    searchAction(opt) {
      ...
      that.loading = true
      productRulegetListByPage(_searchParam)
        .then((res) => {
          that.loading = false
