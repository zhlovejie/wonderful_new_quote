<template>
  ...
  <div class="container-list-wrapper">
    <div class="search-wrapper">
      ...
      <a-button class="a-button" :loading="btnDownloadLoading" v-if="$auth('salaryRealityPercentageHis:download')" type="primary" icon="download" @click="doAction('download')">下载</a-button>
    </div>
...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'commission-detail-by-person') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    searchAction(opt) {
      ...
      that.loading = true
      realityPercentageHisList(_searchParam)
        .then((res) => {
          that.loading = false
    ...
    downloadAction() {
      ...
      exportExcelDatas(1, that.getSearchParams())
        .then((res) => {
