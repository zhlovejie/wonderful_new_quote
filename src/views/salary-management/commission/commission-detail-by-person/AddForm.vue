...
export default {
  ...
  methods:{
    init(opt){
      const that = this
      that.spinning = true
      realityPercentageHisDetail(opt).then(res => {
        that.spinning = false
    ...
    query(type,record){
      ...
      this.title = `${staticsDate.slice(0,7)}月【${record.departmentName} ${record.stationName} ${record.userName}】提成明细`
      this.$nextTick(() => {
