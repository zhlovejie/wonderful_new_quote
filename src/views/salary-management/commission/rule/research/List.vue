    ...
    <div class="search-wrapper">
      ...
      <template v-if="$auth('other:add')">
        ...
        <a-dropdown style="float: right; margin-right: 10px">
          <a-button type="primary" @click="doAction('intelligence')">智能产品提成 </a-button>
        </a-dropdown>
      </template>
...
import { researchs_list, researchs_del, getSalaryIntelligent } from '@/api/bonus_management'
import AddForm from './module/Formadd'
import Intelligence from './module/Approval'
...
export default {
  ...
  methods: {
    ...
    confirmDelete(record) {
      let that = this
      researchs_del({ id: record.id }).then((res) => {
        if (res.code === 200) {
          this.searchAction()
    ...
    doAction(type, record) {
      if (type === 'intelligence') {
        getSalaryIntelligent().then((res) => {
          console.log(res.data)
          if (res.code === 200 && res.data === null) {
            this.$refs.intelligence.query('add',null)
          } else {
            this.$refs.intelligence.query('edit-salary',res.data)
          }
        })
      } else {
