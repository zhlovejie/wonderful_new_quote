      ...
      <a-form :form="form" class="becoming-form-wrapper">
        <a-row>
          <a-col :span="24" class="basic-tit" justify="center" align="middle"
            >{{ record.staticsDate }}非销售提成奖金统计表</a-col
          >
        </a-row>
...
<script>
import { MonthInstanceApply, getOaSalaryNSalerBounsApplyDetail } from '@/api/bonus_management'
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          align: 'center',
          title: '基本工资(元)',
          dataIndex: 'baseSalaryDecimal',
          key: 'baseSalaryDecimal',
        },
        {
          align: 'center',
          title: '提成金额(元)',
  ...
  methods: {
    ...
    fillData() {
      let that = this
      that.$nextTick(() => {
        getOaSalaryNSalerBounsApplyDetail({ applyId: that.record.id }).then((res) => {
          this.saleCustomers = res.data
          this.percentageRetio = `总提成（元）: ${that.record.allAmount}`
