<template>
  <!-- 通讯申请 -->
  <div class="wdf-custom-wrapper" id="salary-subsidy-communication">
    <div class="search-wrapper">
      <a-form layout="inline">
        ...
        <a-button
          v-if="$auth('salary-subsidy-communication:add')"
          style="float: right"
          type="primary"
          icon="plus"
...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'salary-subsidy-communication') {
          this.init()
        }
      },
      immediate: true,
  ...
  mounted() {
    let that = this
    let ele = document.querySelector('#salary-subsidy-communication')
