<template>
  <!-- 工伤补贴申请 -->
  <div class="wdf-custom-wrapper" id="salary-subsidy-industrial-accident">
    <div class="search-wrapper">
      <a-form layout="inline">
        ...
        <a-button
          v-if="$auth('salary-subsidy-industrial-accident:add')"
          style="float: right"
          type="primary"
          icon="plus"
    ...
    <div class="main-wrapper">
      <a-tabs :activeKey="String(activeKey)" defaultActiveKey="0" @change="tabChange">
        ...
        <template v-if="$auth('salary-subsidy-industrial-accident:approval')">
...
import { InjuryApplyDel, InjuryApplyPageList, InjuryApplyRevocation } from '@/api/salaryManagement'
...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'salary-subsidy-industrial-accident') {
          this.init()
        }
      },
      immediate: true,
  ...
  mounted() {
    let that = this
    let ele = document.querySelector('#salary-subsidy-industrial-accident')
  ...
  methods: {
    ...
    searchAction(opt = {}) {
      ...
      that.loading = true
      InjuryApplyPageList(_searchParam)
        .then((res) => {
          that.loading = false
    ...
    doAction(actionType, record) {
      ...
      if (['view', 'add', 'edit', 'approval'].includes(actionType)) {
        ...
        InjuryApplyRevocation({ id: record.id })
          .then((res) => {
            that.$message.info(res.msg)
            that.searchAction()
          })
