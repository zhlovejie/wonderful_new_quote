<template>
  <!-- 其他申请 -->
  <div class="wdf-custom-wrapper" id="salary-subsidy-other">
    <div class="search-wrapper">
      <a-form layout="inline">
        ...
        <a-button
          v-if="$auth('salary-subsidy-other:add')"
          style="float: right"
          type="primary"
          icon="plus"
...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'salary-subsidy-other') {
          this.init()
        }
      },
      immediate: true,
  ...
  mounted() {
    let that = this
    let ele = document.querySelector('#salary-subsidy-other')
  ...
  methods: {
    ...
    searchAction(opt = {}) {
      ...
      that.loading = true
      OtherApplyPageList(_searchParam)
        .then((res) => {
          that.loading = false
