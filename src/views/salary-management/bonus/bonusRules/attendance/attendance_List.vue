...
<script>
import { attendance_List, attendance_delete } from '@/api/bonus_management'
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          title: '出勤天数(天)',
          key: 'onDutyDays',
          dataIndex: 'onDutyDays',
          scopedSlots: { customRender: 'onDutyDays' },
        },
        {
          align: 'center',
          title: '奖金(元)',
        ...
        },
        {
          align: 'center',
          title: '条件',
          dataIndex: 'maxLeaveHours',
          key: 'maxLeaveHours',
          scopedSlots: { customRender: 'maxLeaveHours' },
        },
        {
          align: 'center',
          title: '备注',
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'bonusRules_attendance') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    deleteRoleInfo(record) {
      let that = this
      attendance_delete(`id=${record.id}`).then((res) => {
        if (res.code === 200) {
          this.searchAction()
