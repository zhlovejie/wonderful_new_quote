<template>
  ...
  <div class="wdf-custom-wrapper">
    <div class="search-wrapper">
      ...
      <a-button style="float:right;" type="primary" icon="plus" @click="doAction('add',null)"  v-if="$auth('salary-subsidy-workyears-rules:add')">新增</a-button>
    </div>
    <div class="main-wrapper">
      ...
      >
        ...
        <div class="action-btns" slot="action" slot-scope="text, record">
          <template v-if="$auth('salary-subsidy-workyears-rules:view')">
            <a type="primary"  @click="doAction('view',record)">查看</a>
          </template>
          <template v-if="$auth('salary-subsidy-workyears-rules:edit')">
            <a-divider type="vertical" />
            <a type="primary" @click="doAction('edit',record)">修改</a>
          </template>
          <template v-if="$auth('salary-subsidy-workyears-rules:edit-record')">
            <a-divider type="vertical" />
            <a type="primary" @click="doAction('edit-record',record)">修改记录</a>
          </template>
          <template v-if="$auth('salary-subsidy-workyears-rules:del')">
            <a-divider type="vertical"  />
            <a-popconfirm
              title="是否要删除此行？"
...
import {
  lengthServiceRulePageList,
  lengthServiceRuleDel,
  lengthServiceRuleDetail,
  lengthServiceRuleAddOrUpdate
...
export default {
  ...
  watch:{
    '$route':{
      handler:function(to,from) {
        if(to.name === 'salary-subsidy-workyears-rules'){
          this.init()
        }
      },
      immediate:true
  ...
  methods:{
    ...
    searchAction(opt = {}){
      ...
      that.loading = true
      lengthServiceRulePageList(_searchParam).then(res => {
        that.loading = false
    ...
    doAction(actionType,record){
      ...
      if(actionType === 'del'){
        console.log(record)
        lengthServiceRuleDel({id:record.id}).then(res =>{
