    ...
    <div class="main-wrapper">
      ...
      >
        ...
        <div slot="type" slot-scope="text, record, index">
          <span>{{ { 1: '工伤', 2: '代班', 3: '大夜班', 4: '中夜班', 5: '延时餐补' }[text] }}</span>
        </div>

        <div class="action-btns" slot="action" slot-scope="text, record">
          <template v-if="$auth('salary-subsidy-rules-record:view')">
            <a type="primary" @click="doAction('view', record)">查看</a>
          </template>
          <template v-if="$auth('salary-subsidy-rules-record:edit')">
            <a-divider type="vertical" />
            <a type="primary" @click="doAction('edit', record)">修改</a>
          </template>
          <template v-if="$auth('salary-subsidy-rules-record:edit-record')">
            <a-divider type="vertical" />
            <a type="primary" @click="doAction('edit-record', record)">修改记录</a>
          </template>
          <template v-if="$auth('salary-subsidy-rules-record:del')">
            <a-divider type="vertical" />
...
import { nightRulePageList, nightRuleDel } from '@/api/salaryManagement'
...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'salary-subsidy-rules-record') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    searchAction(opt = {}) {
      ...
      that.loading = true
      nightRulePageList(_searchParam)
        .then((res) => {
          that.loading = false
    ...
    doAction(actionType, record) {
      ...
      if (actionType === 'del') {
        console.log(record)
        nightRuleDel({ id: record.id }).then((res) => {
