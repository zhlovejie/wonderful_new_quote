  ...
  <div class="product-info-wrapper">
    ...
    >
      ...
      <template slot="productName" slot-scope="text, record">
        <a-form-item>
          <a-input :disabled="isNormal" read-only="read-only" :value="record.productName" />
        </a-form-item>
      </template>
    ...
    <a-row justify="start" align="middle">
      ...
      <a-col class="col-mount" :span="24" v-if="isNormal">
...
export default {
  ...
  methods: {
    async init() {
      ...
      if (__source.length > 0) {
        this.totalAmount = __source.reduce((calc, item) => {
          return calc + (this.isTax ? parseFloat(item.taxAmount) : parseFloat(item.oneMoney))
        }, 0)
        // 运费的显示和计算统一放在产品变动部分，非变动部分金额计算 不含运费
        //this.totalAmount = this.isNormal ? this.totalAmount : this.totalAmount + parseFloat(this.freightCharge)
        //this.totalAmount = this.totalAmount
        this.chineseTotalAmount = await this.totalMmount(this.totalAmount)
      } else {
        this.totalAmount = this.params.totalAmount
        this.chineseTotalAmount = this.params.chineseTotalAmount
      }

      this.dataSource = __source
    ...
    calcNumber(item) {
      ...
      let oneMoney = count * unitPrice //商品价格是含税单价，不用计算税率了
