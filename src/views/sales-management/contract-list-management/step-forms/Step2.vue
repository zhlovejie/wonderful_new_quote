        ...
        <a-row justify="start" align="middle">
          <a-col :span="24">
            <div v-if="freightType === 0">
              <span v-if="isTax === 0"> 运费：&nbsp;{{ freightCharge | moneyFormatNumber }} </span>
              <span v-else>
                运费：&nbsp;{{
                  Math.ceil(freightCharge * 1.13) | moneyFormatNumber
                }}&nbsp;&nbsp;包含&nbsp;&nbsp;(&nbsp;运费：{{
                  freightCharge | moneyFormatNumber
                }}&nbsp;&nbsp;运费税率：&nbsp;13%&nbsp; )
              </span>
            </div>
          </a-col>
    ...
    async nextStep(status) {
      ...
      if (that.$parent.routeParams.action === 'split') {
        ...
        if (low && ispriceC !== false) {
          await copyCPriceDesc({ id: this.queryonedata.id, lowPriceDesc: low }).then((res) => {})
        } else {
          await copyCPriceDesc({ id: this.queryonedata.id }).then((res) => {})
        }
