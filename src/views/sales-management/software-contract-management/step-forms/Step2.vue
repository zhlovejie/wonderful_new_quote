  ...
  <div class="content-wrap">
    ...
    <div class="form wdf-form">
      <template v-if="this.$parent.routeParams.action === 'see'">
        <ProductC ref="productCommon" :params="productCommonParams"></ProductC>
      </template>
      <template v-else>
...
export default {
  name: 'Step2',
  components: { ProductCommon, ProductC },
  props: {
    queryonedata: {
      type: Object,
      default: function () {
  ...
  methods: {
    ...
    getContractInfo(params) {
      return getSoftwareContractQueryOne(params)
        .then((res) => {
          return res.data
        })
        .catch((error) => {
    ...
    async nextStep(status) {
      ...
      saveSoftwareContractProduct(products)
        .then((res) => {
