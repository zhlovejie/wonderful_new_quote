      ...
      <tbody>
        ...
        <template v-if="queryOneData && queryOneData.products && Array.isArray(queryOneData.products)">
          <tr v-for="product in queryOneData.products" :key="product.id">
            <td style="width: 30%">
              {{ product.productName }}-{{ parseInt(product.productType) === 0 ? '标准版' : '定制版' }}
            </td>
            <td style="width: 70%">
              <a-row>
                <template v-if="parseInt(product.productType) === 0">
                  <a-checkbox-group
                    :options="product.plainOptions || []"
                    :value="product.checkedList || []"
                    :disabled="isView"
                    @change="onPointChange(product.id, $event)"
                  />
                </template>
                <template v-if="parseInt(product.productType) === 1">
                  <a-textarea
                    placeholder="请输入定制需求"
                    :rows="4"
                    :value="product.productFunPoint"
                    :disabled="isView"
                    @change="onProductFunPointChange(product.id, $event)"
                  />
                </template>
              </a-row>
            </td>
...
export default {
  ...
  methods: {
    ...
    nextStep(status) {
      ...
      } else if (that.$parent.routeParams.action === 'see') {
        console.log('修改模式 添加参数 contractModifyFlag：1')
        params.contractModifyFlag = 1
      }
      // 校验成功，保存填写的信息，请求后端接口存起来，进入下一个页面
      saveProductPoints(params)
        .then((res) => {
