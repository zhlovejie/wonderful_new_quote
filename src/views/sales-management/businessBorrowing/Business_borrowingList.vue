...
import { businessList, businessrevocation, businessdelete } from '@/api/agencyContract'
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'Business_borrowingList') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    preview(record) {
      this.$router.push({
        ...
        params: { id: record.id, action: 'view', from: 'Business_borrowingList' },
      })
    },
    handleSaveOk() {
      this.searchAction()
