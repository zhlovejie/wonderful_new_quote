...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'strategic-cooperation-agreement-list') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    searchAction(opt) {
      ...
      that.loading = true
      cooperationProtocolPageList(_searchParam)
        .then((res) => {
          that.loading = false
    ...
    doAction(type, record) {
      let that = this
      if (type === 'reback') {
        cooperationProtocolRevocation(`id=${record.id}`)
          .then((res) => {
      ...
      if (type === 'del') {
        cooperationProtocolDelete(`id=${record.id}`)
          .then((res) => {
            that.$message.info(res.msg)
            if (+res.code === 200) {
              that.searchAction()
