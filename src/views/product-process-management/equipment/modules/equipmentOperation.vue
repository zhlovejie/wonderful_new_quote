    ...
    <div class="description-document-search-wrapper">
      <a-form layout="inline">
        ...
        <div class="action-wrapper" style="float: right;margin-bottom：10px">
          <a-form-item v-if="types !== 'view'">
            <template>
              <!-- : 1、设备操作规程 2、保养操作规程 3、耗材 -->
              <a-button type="primary" icon="plus" @click="handleAction('add', null, 1)">上传文件</a-button>
            </template>
          </a-form-item>
        </div>
      </a-form>
...
export default {
  ...
  methods: {
    ...
    query(type, record) {
      this.types = type
      if (type !== 'add') {
        this.dataSource = record.equipmentOperations
      }
    },
    handleAction(type, record) {
      ...
      if (['add', 'edit'].includes(type)) {
        let _record = record
          ...
          : {}
        that.$refs.toolBoxCommonUploadForm.query(type, _record, 1)
      } else if (type === 'del') {
