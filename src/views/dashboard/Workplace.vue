          ...
          >
            <a slot="extra">
              <a-month-picker
                ...
                style="width: 200px"
                :defaultValue="moment()"
                @change="(m, str) => (statiticsMonthDate = str)"
              />
            </a>
...
import { getPushMsg, getPushMsgList, getPushShareMsg } from '@/api/common' //消息
...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      await getPushMsgList().then((res) => {
        let tabListSysInfo = [...this.tabListSysInfo]
        ...
        this.tabListSysInfo = tabListSysInfo
      })

      await this.initPushMsgCount()
    },
    fetchNoticeTop4() {
      let that = this
      NoticeList({ current: 1, size: 4 }).then((res) => {
    ...
    async initPushMsgCount() {
      ...
      let queue = that.tabListSysInfo.map((item) => {
        let isShare = item.key.startsWith('share_')
        let api = isShare ? getPushShareMsg : getPushMsg
        let params = { msgType: isShare ? item.key.replace('share_', '') : item.key, userId }
