  ...
  <div class="wdf-custom-wrapper" id="attendance-leave-apply">
    <div class="search-wrapper">
      <a-form layout="inline">
        ...
        <a-form-item>
          <a-range-picker style="width:280px;" @change="rangePickerChange" />
        </a-form-item>
        <a-form-item>
          <a-button
            class="a-button"
        ...
        <a-button
          v-if="$auth('annualMeetingSetting:add')"
          style="float:right;"
          type="primary"
          icon="plus"
    ...
    <div class="main-wrapper">
      <a-tabs :activeKey="String(activeKey)" defaultActiveKey="0" @change="tabChange">
        ...
        <template v-if="$auth('annualMeetingSetting:approval')">
      ...
      >
        ...
        <div slot="name" slot-scope="text, record">年会福利</div>
...
import {
  ...
  annualMeetingSettingDel,
  annualMeetingSettingDetail,
  annualMeetingSettingList,
  annualMeetingSettingAddOrUpdate,
  annualMeetingSettingRevocation,
...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'welfare-management-welfare-annual-meeting-config') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    searchAction(opt = {}) {
      ...
      that.loading = true
      annualMeetingSettingList(_searchParam)
        .then((res) => {
          that.loading = false
    ...
    doAction(actionType, record) {
      ...
      if (['view', 'add', 'edit', 'approval'].includes(actionType)) {
        ...
        annualMeetingSettingDel(`id=${record.id}`)
          .then((res) => {
        ...
        annualMeetingSettingRevocation(`id=${record.id}`)
          .then((res) => {
            that.$message.info(res.msg)
            that.searchAction()
          })
