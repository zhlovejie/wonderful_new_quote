  ...
  <div class="wdf-custom-wrapper" id="attendance-leave-apply">
    ...
    <div class="main-wrapper">
      <a-tabs :activeKey="String(activeKey)" defaultActiveKey="0" @change="tabChange">
        ...
        <template v-if="$auth('caringSettingRecord:approval')">
...
import {
  caringSettingRecordApprove,
  caringSettingRecordDel,
  caringSettingRecordDetail,
  caringSettingRecordList,
  caringSettingRecordRevocation,
...
export default {
  ...
  watch: {
    $route: {
      handler: function (to, from) {
        if (to.name === 'welfare-management-employee-care-config-record') {
          this.init()
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    searchAction(opt = {}) {
      ...
      that.loading = true
      caringSettingRecordList(_searchParam)
        .then((res) => {
          that.loading = false
    ...
    doAction(actionType, record) {
      ...
      if (['view', 'add', 'edit', 'approval'].includes(actionType)) {
        ...
        caringSettingRecordDel(`id=${record.id}`)
          .then((res) => {
        ...
        caringSettingRecordRevocation(`id=${record.id}`)
          .then((res) => {
            that.$message.info(res.msg)
            that.searchAction()
          })
