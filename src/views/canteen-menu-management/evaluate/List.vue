<template>
  ...
  <div class="wdf-custom-list-wrapper">
    <div class="search-wrapper">
      ...
      <a-select
        placeholder="满意度"
        v-model="searchParam.satisfaction"
        :allowClear="true"
        style="width: 140px"
      >
        <a-select-option
          v-for="(item,index) in menus.satisfaction"
          :key="index"
          :value="item.id"
      ...
      <a-select
        placeholder="服务态度"
        v-model="searchParam.attitude"
        :allowClear="true"
        style="width: 140px"
      >
        <a-select-option
          v-for="(item,index) in menus.attitude"
          :key="index"
          :value="item.id"
        >{{item.txt}}</a-select-option>
      </a-select>
      <a-select
        placeholder="就餐环境"
        v-model="searchParam.environment"
        :allowClear="true"
        style="width: 140px"
      >
        <a-select-option
          v-for="(item,index) in menus.environment"
          :key="index"
          :value="item.id"
        >{{item.txt}}</a-select-option>
      </a-select>
      <a-select
        placeholder="饭菜分量"
        v-model="searchParam.weight"
        :allowClear="true"
        style="width: 140px"
      >
        <a-select-option
          v-for="(item,index) in menus.weight"
          :key="index"
          :value="item.id"
        >{{item.txt}}</a-select-option>
      </a-select>
