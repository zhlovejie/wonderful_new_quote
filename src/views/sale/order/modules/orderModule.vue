  ...
  <a-spin :spinning="spinning">
    <div>
      <div class="top-right clearfix">
        <a-button class="fl-r" v-if="!show" type="primary" @click="addClick" :loading="loading" icon="check"
          >提交</a-button
        >
        <a-button class="fl-r" type="primary" @click="goBackPricing" icon="backward">返回</a-button>
        <a-button
          v-show="record.approvalStatus == 2 && $auth('distributionContract:changeProduct')"
          class="fl-r"
          type="primary"
          @click="splitOrderAction"
          icon="backward"
          >拆分订单</a-button
        >
      </div>
        ...
        >
          ...
          <template slot="deliveryDate" slot-scope="text, record">
            <a-date-picker
              :disabled="show"
              format="YYYY-MM-DD"
              :value="record.deliveryDate ? moment(record.deliveryDate) : ''"
          ...
          <div slot="productStatus" slot-scope="text, record">
            <a-select :disabled="show" style="width: 100px" v-model="record.productStatus">
              <a-select-option v-for="item in record.productStatusList" :key="item.id" :value="item.id"
                >{{ item.name }}
              </a-select-option>
            </a-select>
          </div>
...
export default {
  ...
  methods: {
    ...
    edit() {
      ...
      this.spinning = true
      orderDetail({ contractId: _contractId })
        .then(res => {
          this.spinning = false
          if (res.code === 200) {
    ...
    splitOrderAction() {
      ...
      this.$router.push({
        ...
        params: { id: this.record.id, show: this.show, action: 'split', from: 'orderModule' }
      })
    },
    // 交货日期的改变
