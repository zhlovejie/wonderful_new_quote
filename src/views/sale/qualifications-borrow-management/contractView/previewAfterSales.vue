        ...
        <div class="contract-main">
          ...
          <div class="main-content-wrap">
            ...
            <div class="content-p p-text-index">
              乙方承揽甲方产品售后日常维护以及辅助安装调试服务，所在<span class="span-underline">{{
                detail.areaName
              }}</span
              >区域内的所有甲方产品，共需要维修费用人民币<span class="span-underline">{{ detail.maintenanceCost }}</span
              >元/年，合同期内如果甲方新增其它服务项目，服务费用按双方另行约定的标准进行计算。
            </div>
            ...
            <div class="content-p p-text-index">
              自 <span class="span-underline">{{ detail.effectiveStart }}</span> 至
              <span class="span-underline">{{ detail.effectiveEnd }}</span> 。
            </div>
            ...
            <div class="content-p p-text-index">
              ...
              每次支付人民币<span class="span-underline">{{ detail.paymentAmount }}</span
              >元，合计年服务费人民币<span class="span-underline">{{ detail.yearCost }}</span> 元。<br />
              4.2售后服务费以汇款的方式打入乙方的公司账户内。（双方约定付款方式仅以公对公的方式）
            </div>
            <div class="content-p">第五条：双方权利义务</div>
            <div class="content-p p-text-index">
              5.1乙方应积极完成合同约定的售后服务内容，维护甲方权益。 5.2甲方应按时向乙方支付相应的服务费用。
...
export default {
  name: 'previewAfterSales-contractView',
  props: {
  ...
  methods: {
    ...
    async init() {
      ...
      afterdetail({ id: that.contractId })
        .then((res) => {
          let data = res.data
          if (data.products) {
