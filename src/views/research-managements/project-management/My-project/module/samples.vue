<template>
  <a-form-model ref="ruleForm" :model="form" :rules="rules" class="routine-addform-wrapper-baseInnerData">
    ...
    <div class="saveOk">
      <a-button type="primary" style="margin-right: 10px" v-if="!isDisabled" @click="validate()">提交审核</a-button>
...
<script>
import { sampleShow, approveProjectStageApply } from '@/api/projectManagement'
...
export default {
  ...
  computed: {
    isDisabled() {
      ...
      return (
        ...
        (this.normalAddForm.isHandle && this.normalAddForm.status !== 17)
      )
    },
  },
  data() {
  ...
  watch: {
    'normalAddForm.schemesData': function (newVal, oldVal) {
  ...
  methods: {
    ...
    init() {
      ...
      if (that.queryOneData) {
        that.$nextTick(() => {
          that.details = {
            showFlag: that.normalAddForm.schemesData.showFlag,
          }
          that.form = {
            ...that.details,
          }
