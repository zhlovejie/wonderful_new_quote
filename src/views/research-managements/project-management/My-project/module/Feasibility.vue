<template>
  <a-form-model ref="ruleForm" :model="form" :rules="rules" class="routine-addform-wrapper-baseInnerData">
    ...
    <a-table
      :columns="columns1"
      :dataSource="form.developmentProjectFeasibilityTestResults"
      :pagination="false"
      size="small"
    >
      <div slot="action" slot-scope="text, record, index">
        <template v-if="!isFeasibility && normalAddForm.status === 5">
      ...
      >
        <div slot="testResult" slot-scope="text, record, index">
          <a-form-model-item
            ...
            :prop="'developmentProjectFeasibilityTestResults.' + index + '.testResult'"
            :rules="{
              required: true,
              message: '请选择是否通过',
            }"
...
export default {
  ...
  computed: {
    isFeasibility() {
      ...
      return (
        ...
        (this.normalAddForm.isHandle && this.normalAddForm.status !== 5) ||
        (this.normalAddForm.status === 5 && this.normalAddForm.FeasibilityData.status === 2)
      )
    },
    //可行性测试测试结果显示问题
    testResults() {
  ...
  data() {
    return {
      ...
      form: {
        period: undefined,
        developmentProjectFeasibilityTestResults: [],
        beginTime: undefined,
  ...
  watch: {
    'normalAddForm.FeasibilityData': function (newVal, oldVal) {
  ...
  methods: {
    resultsTest() {
      let that = this
      let value = {}
      value.id = that.normalAddForm.FeasibilityData.id
      value.projectId = that.normalAddForm.allInfo.id
      value.developmentProjectFeasibilityTestResults = that.form.developmentProjectFeasibilityTestResults
      value.resultRemark = that.form.resultRemark
      value.summary = that.form.summary
      updateTestesult(value)
        .then((res) => {
          if (res.code === 200) {
    ...
    downloadAction() {
      let that = this
      that.spinning = true
      savaExportExcel({ testId: that.normalAddForm.FeasibilityData.id })
        .then((res) => {
          that.spinning = false
          console.log(res)
    ...
    downloadActionApp() {
      let that = this
      that.spinning = true
      exportExcelApproval({ testId: that.normalAddForm.FeasibilityData.id })
        .then((res) => {
          that.spinning = false
          console.log(res)
          if (res instanceof Blob) {
    ...
    init() {
      ...
      if (that.queryOneData) {
        that.dataSource = that.normalAddForm.FeasibilityData.developmentProjectFeasibilityTestResults
        that.$nextTick(() => {
          that.details = {
            period: that.normalAddForm.FeasibilityData.period,
            developmentProjectFeasibilityTestResults:
              that.normalAddForm.FeasibilityData.developmentProjectFeasibilityTestResults || [],
            beginTime:
              that.normalAddForm.FeasibilityData.beginTime !== null
                ? moment(that.normalAddForm.FeasibilityData.beginTime)
                : null,
            contentRemark: that.normalAddForm.FeasibilityData.contentRemark,
            summary: that.normalAddForm.FeasibilityData.summary,
            resultRemark: that.normalAddForm.FeasibilityData.resultRemark,
          }
          that.form = {
            ...that.details,
          }
    ...
    moment,
    validate(opt) {
      if (this.form.developmentProjectFeasibilityTestResults.length === 0) {
    ...
    preservation(status) {
      ...
      value.developmentProjectFeasibilityTestResults = that.FeasibilityTest.developmentProjectFeasibilityTestResults
    ...
    handleSaveOk(data) {
      this.form.developmentProjectFeasibilityTestResults.push(data)
    },
    handleAction(type, record) {
      ...
      if (['add'].includes(type)) {
        ...
        that.form.developmentProjectFeasibilityTestResults = that.form.developmentProjectFeasibilityTestResults.filter(
