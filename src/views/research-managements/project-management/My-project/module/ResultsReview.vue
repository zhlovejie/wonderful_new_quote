<template>
  <a-form-model ref="ruleForm" :model="form" :rules="rules" class="routine-addform-wrapper-baseInnerData">
    ...
    <a-table
      :columns="columns1"
      :dataSource="form.developmentProjectFeasibilityTestReviewFiles"
      :pagination="false"
      size="small"
    >
      <div slot="action" slot-scope="text, record, index">
        <a @click="delSee(record.fileUrl)">查看</a>
        <template v-if="!isResults && normalAddForm.status === 6">
...
<script>
import { reviewSaveAndUpdate, approveProjectStageApply } from '@/api/projectManagement'
...
export default {
  ...
  computed: {
    isResults() {
      ...
      return (
        ...
        (this.normalAddForm.isHandle && this.normalAddForm.status !== 6)
      )
    },
    Conference() {
      //会议记录显示
      return JSON.stringify(this.normalAddForm.ResultsData) == '{}'
    },
  },
  data() {
    return {
      ...
      form: {
        remark: undefined,
        developmentProjectFeasibilityTestReviewFiles: [],
  ...
  watch: {
    'normalAddForm.ResultsData': function (newVal, oldVal) {
  ...
  methods: {
    ...
    init() {
      ...
      if (that.queryOneData) {
        that.dataSource = that.normalAddForm.ResultsData.developmentProjectFeasibilityTestReviewHisVos
        that.Noshow =
          that.normalAddForm.ResultsData.developmentProjectFeasibilityTestReviewHisVos[0].reviewResult === 0
            ? true
            : false
        that.$nextTick(() => {
          that.details = {
            remark: that.normalAddForm.ResultsData.remark,
            developmentProjectFeasibilityTestReviewFiles:
              that.normalAddForm.ResultsData.developmentProjectFeasibilityTestReviewHisVos[0]
                .developmentProjectFeasibilityTestReviewFileVos || [],
            reviewResult:
              that.normalAddForm.ResultsData.developmentProjectFeasibilityTestReviewHisVos[0].reviewResult.toString(),
            nextReviewTime:
              that.normalAddForm.ResultsData.developmentProjectFeasibilityTestReviewHisVos[0].nextReviewTime !== null
                ? moment(that.normalAddForm.ResultsData.developmentProjectFeasibilityTestReviewHisVos[0].nextReviewTime)
                : null,
            reason: that.normalAddForm.ResultsData.developmentProjectFeasibilityTestReviewHisVos[0].reason,
          }
          that.form = {
            ...that.details,
          }
    ...
    moment,
    validate() {
      if (this.form.developmentProjectFeasibilityTestReviewFiles.length === 0) {
    ...
    preservation(status) {
      ...
      that.validate()
      if (that.details) {
        value.id = this.normalAddForm.ResultsData.id
      }
      ...
      that.developmentProjectDesignReview.nextReviewTime =
        that.developmentProjectDesignReview.reviewResult === '0'
          ? moment(that.developmentProjectDesignReview.nextReviewTime).format('YYYY-MM-DD hh:mm:ss')
          : undefined
      value.developmentProjectFeasibilityTestReviewHis = that.developmentProjectDesignReview
      reviewSaveAndUpdate(value)
        .then((res) => {
          if (res.code === 200) {
            that.$message.info(res.msg)
            that.$router.go(-1)
    ...
    handleSaveOk(data) {
      this.form.developmentProjectFeasibilityTestReviewFiles.push(data)
    },
    changeClient(e) {
    ...
    handleAction(type, record) {
      ...
      if (['add'].includes(type)) {
        ...
        that.form.developmentProjectFeasibilityTestReviewFiles =
          that.form.developmentProjectFeasibilityTestReviewFiles.filter((item) => item.fileName !== record.fileName)
      }
    },
  },
}
