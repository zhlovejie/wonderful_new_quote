  ...
  <div class="adjust-apply-list-wrapper">
    <div class="search-wrapper">
      ...
      <a-button
        ...
        >查询</a-button
      >
      <template v-if="$auth('shuttleBus:add')">
    ...
    <div class="main-wrapper">
      <a-tabs :activeKey="String(activeKey)" defaultActiveKey="0" @change="tabChange">
        ...
        <template v-if="$auth('shuttleBus:list')">
          <a-tab-pane tab="待审批" key="1" />
          <a-tab-pane tab="已审批" key="2" />
        </template>
      </a-tabs>
      <a-table
        v-if="$auth('shuttleBus:lists')"
        :columns="columns"
      ...
      >
        ...
        <div class="action-btns" slot="action" slot-scope="text, record">
          ...
          <template v-if="activeKey === 0">
            <template v-if="$auth('shuttleBus:view')">
              <a type="primary" @click="doAction('view', record)">查看</a>
            </template>
            <template v-if="record.status === 1 && +record.createdId === +userInfo.id">
              <a-divider type="vertical" />
              <template v-if="$auth('shuttleBus:Withdraw')">
...
export default {
  ...
  methods: {
    ...
    confirmDelete(record) {
      let that = this
      shuttle_Bus_del(`id=${record.id}`).then((res) => {
        if (res.code === 200) {
          this.searchAction()
    ...
    confirmWithdraw(record) {
      let that = this
      shuttle_Bus_withdraw({ id: record.id }).then((res) => {
        this.searchAction()
        that.$message.info(res.msg)
      })
    },
