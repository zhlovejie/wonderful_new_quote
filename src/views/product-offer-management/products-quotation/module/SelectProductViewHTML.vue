  ...
  <div class="products-quotation-view-wrapper" id="products-quotation-view-wrapper">
    <div class="products-quotation-view-item">
      <table
        ...
        v-if="productInfo.optInfo && productInfo.__config && productInfo.__config.showTitle"
      >
        <caption>
          产品参数
        </caption>
        ...
        <tr v-if="!isPriceViewCost">
          ...
          <td>
            <a-row>
              <a-col :span="24">
                <a-select style="width: 140px" placeholder="选择产品价格" v-model="priceSelect" @change="priceChange">
                  <a-select-option v-for="v in priceList" :value="v.price" :key="v.id">{{
      ...
      <p style="padding: 10px; font-size: 125%; background-color: #f5f5f5" v-if="isPriceViewCost">
        总成本价：{{ priceAll.totalPrice.price | moneyFormatNumber(0) }}
      </p>
    </div>
  </div>
</template>
  ...
  computed: {
    priceList() {
      if (this.productInfo && this.productInfo.__config && this.productInfo.__config.costPrice) {
        return [...this.productInfo.__config.costPrice]
      }
      return []
    },
  ...
  methods: {
    ...
    optStandHTML() {
      ...
      this.productInfo.optStand.map((p, idx) => {
        html += `
          <tr>
            <td style="padding:5px 0;width:35%;">${p.itemName}</td>
            <td style="padding:15px;width:65%;text-align:left;">${p.introduction || ''}</td>
          </tr>
        `
      })
      html += '</table>'
