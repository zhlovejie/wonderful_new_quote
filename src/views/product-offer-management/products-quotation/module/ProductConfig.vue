<template>
  <div class="product-config-wrapper" style="width: 1000px; margin: 0 auto">
    ...
    <template v-if="optControl.length > 0">
        ...
        <tr>
          ...
          <td>
            <a-select style="width: 100%" :defaultValue="optControlSelectedDefault" @change="controlChangeHandler">
              <a-select-option v-for="(item, index) in optControl" :key="index" :value="item.id">{{
                item.itemName
              }}</a-select-option>
            </a-select>
          </td>
      ...
      <table
        class="custom-table custom-table-border c-control-color"
        v-if="controlResult.optStand && controlResult.optStand.length > 0"
      >
        <tr>
      ...
      <table
        class="custom-table custom-table-border c-control-color"
        v-if="controlResult.optSelect && controlResult.optSelect.length > 0"
      >
        <tr>
            ...
            <table class="custom-table custom-table-border" style="margin-bottom: 0">
              ...
              <tr v-for="(item, index) in controlResult.optChoice" :key="index">
                ...
                <td>
                  ...
                  >
                    <a-select-option v-for="(subItem, index) in item.dataSource" :key="index" :value="subItem.id">
                      {{ subItem.itemName }}
                    </a-select-option>
                  </a-select>
                </td>
        ...
        <td style="padding: 0">
          ...
          <table class="custom-table custom-table-border" style="margin-bottom: 0">
            ...
            <tr v-for="(item, index) in optSelect" :key="item.id">
              ...
              <td>
                <a-checkbox :checked="item.checked || false" @change="optSelectCheckChange(item.id, $event)" />
              </td>
            </tr>
          </table>
        </td>
...
<script>
import { priceAdjustProductConfigDetail, priceAdjustZktConfigDetail } from '@/api/productOfferManagement'
...
export default {
  ...
  methods: {
    ...
    selectedHandler(result) {
      ...
      let { name, model, productPic, type, price, aprice, bprice, cprice, retailPrice, remarks, sysConfigList } = result
      that.optInfo = {
        name,
        model,
        productPic,
    ...
    calcItems(type = '4d0') {
      ...
      result.reduce((calc, item) => {
        console.log(
          `${item.itemName} 成本价：${item.price} A价：${item.aprice} B价：${item.bprice} C价：${item.cprice} 销售价：${item.retailPrice}`
        )
      ...
      if (optStand.length >= 0) {
        ...
        if (type === '2d0') {
          ...
          priceResult.retailPrice +=
            (parseFloat(optInfo.retailPrice) || 0) - (parseFloat(optStandUnCheckedPrice.retailPrice) || 0)
        }
      }
      if (control_optStand.length > 0) {
        console.log(
          `【中控标配】 ${control_optInfo.name} 成本价：${control_optInfo.price} A价：${control_optInfo.aprice} B价：${control_optInfo.bprice} C价：${control_optInfo.cprice} 销售价：${control_optInfo.retailPrice}`
        )
    ...
    query(id) {
      ...
      priceAdjustProductConfigDetail({ id: id, isPrice: true })
        .then(res => {
          ...
          let { optStandData, optSelectData, optChoiceData, optControlData } = that.productFormatData(
            res.data.sysConfigList
          )
...
.product-config-wrapper .custom-table th,
.product-config-wrapper .custom-table td {
  position: relative;
  padding: 10px;
}

.c-stand-color {
  background-color: rgba(170, 210, 240, 0.1);
}
.c-control-color {
  background-color: rgba(170, 210, 240, 0.5);
}
.c-select-color {
