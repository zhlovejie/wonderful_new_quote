...
<script>
import { priceAdjustZktConfigList, priceAdjustZktConfigDelete } from '@/api/productOfferManagement'
...
export default {
  ...
  methods: {
    ...
    searchAction(opt) {
      ...
      that.loading = true
      priceAdjustZktConfigList(_searchParam)
        .then((res) => {
          that.loading = false
    ...
    doAction(type, record) {
      let that = this
      if (type === 'del') {
        priceAdjustZktConfigDelete({ id: record.id })
          .then((res) => {
