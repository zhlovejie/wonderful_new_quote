<template>
  ...
  <div class="reserve-list-wrapper">
    <div class="search-wrapper">
      ...
      <a target="_blank" class="a-button ant-btn ant-btn-primary" href="https://www.delanshi.cn/images/cloud/20200409/人才储备模板cde1f372-2758-4724-b8a5-6004b411df2f.xlsx">下载模板</a>
    ...
    <div class="main-wrapper">
      ...
      >
        ...
        <div slot="isInterviewed" slot-scope="text, record">
          <a type="primary" @click="doAction('interviewed',record)">
            {{ getInterviewedTxt(record.isInterviewed)}}
          </a>
        </div>
        ...
        <div class="action-btns" slot="action" slot-scope="text, record">
          ...
          <a-divider type="vertical" v-if="record.resume" />
          <a type="primary" v-if="record.resume" target="_blank" :href="record.resume">下载简历</a>
...
export default {
  ...
  methods:{
    ...
    searchAction(opt = {}){
      ...
      that.loading = true
      getReserveList(_searchParam).then(res => {
        that.loading = false
    ...
    doAction(actionType,record){
      ...
      }else if(actionType === 'interviewed'){ //面试邀请
        //邀请过了，忽略
      ...
      else if(actionType === 'upload'){ //上传简历
    ...
    getInterviewedTxt(type){//请选择是否邀请面试
      let map = {
        0:'否',
        1:'是'
      }
    ...
    openCamera(){
      let isSupportCamera = navigator.mediaDevices && navigator.mediaDevices.getUserMedia || navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia
      if(isSupportCamera){
