        ...
        <table class="custom-table custom-table-border">
              ...
              <a-form-item>
                <a-input
                  v-decorator="[
                    ...
                    { rules: [{ required: isAddDepartment, message: '部门名称' }] },
                  ]"
                />
              </a-form-item>
            </td>
            ...
            <td>
              <a-form-item>
                <a-select
                  ...
                  v-decorator="[
                    'department.parentId',
                    { rules: [{ required: isAddDepartment, message: '请选择上级部门' }] },
                  ]"
                >
                  <a-select-option v-for="item in addDepSelectDataSource" :key="item.id" :value="item.id">
                    {{ item.departmentName }}
                  </a-select-option>
                </a-select>
              </a-form-item>
          ...
          <tr v-if="isAddDepartment" style="background-color: #faebd7">
            ...
            <td>
              <a-form-item>
                <a-input
                  read-only
                  @click="openSystemUsers('department.leaderName')"
                  v-decorator="[
                    'department.leaderName',
                    { rules: [{ required: isAddDepartment, message: '所属上级负责人' }] },
                  ]"
                />
              </a-form-item>
              <a-form-item>
            ...
            <td>
              <a-form-item>
                <a-select
                  placeholder="选择部门等级"
                  v-decorator="['department.level', { rules: [{ required: isAddStation, message: '请选择部门等级' }] }]"
                >
          ...
          <tr v-if="isAddDepartment" style="background-color: #faebd7">
            ...
            <td colspan="3">
              <a-form-item>
                <a-input
                  placeholder="部门备注"
                  v-decorator="['department.remarks', { rules: [{ required: false, message: '新增部门备注' }] }]"
                />
              </a-form-item>
            </td>
          </tr>
          ...
          <tr v-show="isAddStation" style="background-color: #faebd7">
            ...
            <td>
              <a-form-item>
                <a-input
                  v-decorator="['station.stationName', { rules: [{ required: isAddStation, message: '岗位名称' }] }]"
                />
              </a-form-item>
            </td>
            <td>上级岗位</td>
            <td>
              <a-form-item>
                ...
                >
                  <a-select-option v-for="item in addPostSelectDataSource" :key="item.id" :value="item.id">
                    {{ item.stationName }}
                  </a-select-option>
                </a-select>
              </a-form-item>
          ...
          <tr v-show="isAddStation" style="background-color: #faebd7">
            ...
            <td>
              <a-form-item>
                ...
                <a-input-number
                  ...
                  v-decorator="[
                    'station.setNum',
                    { rules: [{ required: isAddStation, message: '请输入岗位设定人数' }] },
                  ]"
                />
              </a-form-item>
            </td>
            ...
            <td>
              <a-form-item>
                <a-select
                  placeholder="选择岗位等级"
                  v-decorator="['station.level', { rules: [{ required: isAddStation, message: '选择岗位等级' }] }]"
                >
          ...
          <tr v-show="isAddStation" style="background-color: #faebd7">
            ...
            <td colspan="3">
              <a-form-item>
                <a-input
                  placeholder="岗位备注"
                  v-decorator="['station.remarks', { rules: [{ required: false, message: '新增岗位备注' }] }]"
                />
              </a-form-item>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <a-form-item>
                <a-input
                  :disabled="isDisabled"
                  v-decorator="['needNum', { rules: [{ required: true, message: '请输入需求人数' }] }]"
                />
              </a-form-item>
            </td>
            <td>到岗日期</td>
            <td>
              <a-form-item>
                <a-date-picker
                  ...
                  v-decorator="[
                    'dutyDate',
                    { initialValue: moment(), rules: [{ required: true, message: '输入到岗日期' }] },
                  ]"
                  format="YYYY-MM-DD"
                />
              </a-form-item>
          ...
          <tr>
            ...
            <td>
              <a-form-item>
                <a-select
                  :disabled="isDisabled"
                  placeholder="选择性别"
                  v-decorator="['needSex', { initialValue: 1, rules: [{ required: false, message: '选择性别' }] }]"
                >
            ...
            <td>
              <a-form-item>
                <a-input
                  :disabled="isDisabled"
                  v-decorator="['needAge', { rules: [{ required: false, message: '请输入年龄要求' }] }]"
                />
              </a-form-item>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <a-form-item>
                <a-select
                  :disabled="isDisabled"
                  v-decorator="['educationId', { rules: [{ required: false, message: '选择学历要求' }] }]"
                  placeholder="选择学历要求"
                >
            ...
            <td>
              <a-form-item>
                <a-input
                  :disabled="isDisabled"
                  v-decorator="['major', { rules: [{ required: false, message: '输入专业' }] }]"
                  placeholder="输入专业"
                />
              </a-form-item>
            </td>
            ...
            <td>
              <a-form-item>
                <a-select
                  ...
                  v-decorator="[
                    'maritalStatus',
                    { initialValue: 0, rules: [{ required: false, message: '选择婚姻状态' }] },
                  ]"
                >
            ...
            <td>
              <a-form-item>
                <a-select
                  ...
                  v-decorator="[
                    'fertilityStatus',
                    { initialValue: 0, rules: [{ required: false, message: '选择生育状态' }] },
                  ]"
                >
            ...
            <td>
              <a-form-item>
                ...
                <a-tooltip>
                  ...
                  <a-input
                    :style="{ width: '100%', color: gtPostBaseSalary ? 'red' : '' }"
                    :disabled="isDisabled"
                    @change="salaryChange"
                    v-decorator="['salary', { rules: [{ required: true, message: '输入建议薪资(元)' }] }]"
                  />
                </a-tooltip>
              </a-form-item>
            </td>
          ...
          <tr>
            ...
            <td>
              <a-form-item>
                <a-select
                  :disabled="isDisabled"
                  placeholder="公司内部有无合适人员"
                  @change="haveInnerFlagChange"
                  v-decorator="[
                    'haveInnerFlag',
                    { initialValue: 0, rules: [{ required: false, message: '公司内部有无合适人员' }] },
                  ]"
                >
                  <a-select-option :value="0">无</a-select-option>
            ...
            <td>
              <a-form-item>
                <a-input
                  :disabled="haveInnerFlag != 1 || isView"
                  placeholder="选择人员名称"
                  read-only
                  style="cursor: pointer"
                  @click="openSystemUsers('innerName')"
                  v-decorator="['innerName']"
                />
              </a-form-item>
              <a-form-item>
            ...
            <td colspan="3">
              <a-form-item>
                <a-select
                  ...
                  v-decorator="[
                    'emergentLevel',
                    { initialValue: 1, rules: [{ required: false, message: '紧急程度' }] },
                  ]"
                >
...
export default {
  ...
  methods: {
    ...
    depChangeHandler(dep_id) {
      ...
      return personnelFullMemberApply_GetStationList({ adverInfoId: that.record.id, departmentId: dep_id }).then(
        (res) => {
    ...
    async handleOk() {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          if (that.haveInnerFlag === 1 && values.innerName === undefined) {
            return that.$message.error('请选择人员名称')
          }
    ...
    async query(type, record) {
      ...
      getPersonAdvertInfoDetail({ advertId: that.record.id }).then((res) => {
        ...
        personnelFullMemberApply_GetStationList({ adverInfoId: that.record.id, departmentId: data.departmentId }).then(
          (res) => {
            that.postSelectDataSource = res.data
            that.addPostSelectDataSource = res.data.map((item) => Object.assign({}, item))
        ...
        oaSalaryInfoStationStandardDetail({ depId: data.departmentId })
          .then((res) => {
            ...
            that.$nextTick(
              () => (that.gtPostBaseSalary = +that.postBaseSalary > 0 && +data.salary > +that.postBaseSalary)
            )
          })
          .catch((err) => (that.postSalaryList = []))
      })
    ...
    submitAction(opt) {
      ...
      that.spinning = true
      approvePersonAdvertInfo(values)
        .then((res) => {
          that.spinning = false
          console.log(res)
