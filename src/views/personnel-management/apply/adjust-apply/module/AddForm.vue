    ...
    <table class="custom-table custom-table-border">
      <tr>
        ...
        <td>
          ...
          <a-form-item>
            <a-input placeholder="员工名" :disabled="isDisabled"  read-only @click="openSystemUsers('applyUserName')" v-decorator="['applyUserName', { rules: [{ required: true, message: '员工名' }] }]"/>
          </a-form-item>
        </td>
        <td>状态</td>
          ...
          <td>
            <a-form-item>
              <a-date-picker 
                :disabled="isDisabled" 
                style="width:100%;" 
                v-decorator="['enterDate',{initialValue:moment(),rules: [{required: true,message: '输入拟入职日期'}]}]" 
                format="YYYY-MM-DD"
              />
            </a-form-item>
          </td>
        ...
        <tr>
          ...
          <td>
            <a-form-item>
              ...
              <a-input-number style="width:100%;" :disabled="isDisabled" :min="0" :step="1" :precision=2  v-decorator="['oldSalary', { rules: [{ required: true, message: '请输入原薪资' }] }]"/>
            </a-form-item>
          </td>
          <td>期望薪资(元)</td>
          <td>
            <a-form-item>
              ...
              <a-input-number style="width:100%;" :disabled="isDisabled" :min="0" :step="1" :precision=2 v-decorator="['expectSalary', { rules: [{ required: true, message: '期望薪资(元)' }] }]"/>
            </a-form-item>
          </td>
        </tr>
        <tr v-if="isSelfApproval">
          ...
          <td colspan="3">
            <a-form-item>
              <a-date-picker 
                :disabled="isDisabled" 
                style="width:100%;" 
                v-decorator="['expectDate',{initialValue:moment(),rules: [{required: true,message: '输入期望生效时间'}]}]" 
                format="YYYY-MM-DD"
              />
            </a-form-item>
          </td>
      ...
      <template v-if="isPostAndSalary">
          ...
          <td>
            <a-form-item>
              <a-date-picker 
                :disabled="isDisabled" 
                style="width:100%;" 
                v-decorator="['nEnterDate',{initialValue:moment(),rules: [{required: true,message: '输入拟入职日期'}]}]" 
                format="YYYY-MM-DD"
              />
            </a-form-item>
          </td>
        ...
        <tr>
          ...
          <td>
            <a-form-item>
              <!-- <a-input :disabled="isDisabled" placeholder="原薪资"  v-decorator="['oldSalary', { rules: [{ required: true, message: '原薪资' }] }]"/> -->
              <a-input-number style="width:100%;" :disabled="isDisabled" :min="0" :step="1" :precision=2 v-decorator="['oldSalary', { rules: [{ required: true, message: '原薪资(元)' }] }]"/>
            </a-form-item>
          </td>
          <td>期望薪资(元)</td>
          <td>
    ...
    async handleOk(){
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          //提交
          that.spinning = true
          apiMap[that.operationStatus](values).then(res =>{
            that.spinning = false
            console.log(res)
    ...
    async query(type,record){
      ...
      if(data.enterDate){
        ...
        that.salary_entryDate = _date.isValid() 
          ? that.moment(data.enterDate).format('YYYY-MM-DD') 
          : ''
      }
