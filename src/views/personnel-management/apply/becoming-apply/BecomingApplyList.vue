      ...
      >
        ...
        <div class="action-btns" slot="action" slot-scope="text, record">
          ...
          <template v-if="activeKey === 0">
            <a type="primary" @click="doAction('view',record)">查看</a>
            <template v-if="$auth('becomingApply:edit') && (record.state === 3 || record.state === 0) && record.showModifyButtonFlag === 1">
...
import {personnelFullMemberApply_PageList} from '@/api/personnelManagement'
...
export default {
  ...
  methods:{
    ...
    searchAction(opt){
      let that = this
      let _searchParam = Object.assign({},{...this.searchParam},{...this.pagination},opt || {},{approveStatus:that.activeKey})
      console.log('执行搜索...',_searchParam)
      that.loading = true
      personnelFullMemberApply_PageList(_searchParam).then(res => {
        that.loading = false
