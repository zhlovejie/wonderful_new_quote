      ...
      >
        ...
        <div class="action-btns" slot="action" slot-scope="text, record">
          <template v-if="activeKey === 0">
            ...
            <template v-if="$auth('probationSurvey:edit') && (record.state === 3 || record.state === 0) && record.showModifyButtonFlag === 1">
...
import {getProbationSurveyList} from '@/api/personnelManagement'
...
export default {
  ...
  methods:{
    ...
    searchAction(opt){
      ...
      that.loading = true
      getProbationSurveyList(_searchParam).then(res => {
        that.loading = false
